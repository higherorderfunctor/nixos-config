...
BRANCH=feat/adds-new-host-insensitive-cube
HOSTNAME=insensitive-cube

# clone the repo
git clone https://github.com/higherorderfunctor/nixos-config.git
cd nixos-config
git checkout "$BRANCH"
...
HOST=root@192.168.2.8

ssh "$HOST" "mkdir -p /mnt/persist/etc/ssh"

nix run nixpkgs#sops -- -d --extract '["secret-ed25519-key"]' \
  ./hosts/<HOST>/secrets/secrets.yaml | \
  ssh "$HOST" 'cat > /mnt/persist/etc/ssh/ssh_host_ed25519_key && chmod 600 /mnt/persist/etc/ssh/ssh_host_ed25519_key'
```
...
*LOCAL (new host):* Run post install steps.

```sh
# clone the repo to the persistent storage
install -d -m 0700 -o $USER -g $USER ~/Documents/projects
