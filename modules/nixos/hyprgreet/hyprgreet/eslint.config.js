...
const eslintPluginJsonc = [
  ...compat.config(EslintPluginJsonc.configs['recommended-with-jsonc']).map((config) => ({
    ...config,
    plugins: {
...
const eslintConfig = [
  ...
  ...eslintPlugPromise,
  {
    ...
    rules: {
       ...
       */
      // runtime hacks
      'import/no-unresolved': ['error', { ignore: ['gtk-4.0'] }],
      ...
      'import/no-extraneous-dependencies': [
        ...
        {
          includeTypes: true,
          devDependencies: ['esbuild.*.ts', 'eslint.config.js'],
        },
      ],
