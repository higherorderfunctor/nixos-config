...
type ArgsWithCancellable<Args extends any[]> = [...args: Args, cancellable: Gio.Cancellable | null];
...
type AsyncMethodKeys<T> = {
  [K in keyof T]: K extends EndsWith<K, '_async'>
    ? T[K] extends (...args: ArgsWithCancellable<infer _Args>) => Promise<infer _R>
      ? K
      : never
    : never;
}[keyof T];

type RemoveLast<T extends any[]> = T extends [...infer Rest, any] ? Rest : never;
