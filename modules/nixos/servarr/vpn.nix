...
in {
  ...
  in
    lib.mkIf cfg.enable (lib.safetyMerge ["boot" "networking" "containers"] (lib.mapNullable (exec-start-pre: [
        (
          _: {
            # HOST CONFIGURATION
            boot.kernel.sysctl = {
              # enables packet logging in containers
              "net.netfilter.nf_log_all_netns" = lib.mkDefault (lib.trace _ true);
