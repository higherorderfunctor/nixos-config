{
  ...
  inherit (lib) mkSafeMerge evalModules;
  ...
  redisModule = import "${inputs.nixpkgs}/nixos/modules/services/databases/redis.nix";
  redisConfig = {
    services.redis = {
      package = pkgs.valkey;
      vmOverCommit = true;
      servers = {
        unbound-cachedb = {
          ...
          settings = {
            ...
            # unixsocket = lib.mkForce redis-sock;
          };
        };
      };
    };
  };
  finalRedisConfig' = evalModules {
    modules = [
      ({config, ...}: {
        config._module.args = {
          inherit lib pkgs;
          config = lib.mkMerge config redisConfig;
        };
      })
      redisModule
    ];
  };
  finalRedisConfig = lib.trace (lib.attrNames finalRedisConfig'.config) finalRedisConfig'.config;
in {
  ...
  in
    lib.mkIf cfg.enable (
      mkSafeMerge ["boot" "networking" "containers"]
      /*
               lib.mapNullable (
      exec-start-pre:
      */
      [
        {
          # containers.servarr-vpn.config.systemd.services.redis = config.containers.servarr-vpn.config.systemd.services.redis-unbound-cachedb;
        }
        {
          # HOST CONFIGURATION
          boot.kernel.sysctl = {
            ...
            in
              lib.mkIf cfg.vpn.enable {
                ...
                config = {
                  ...
                  config = mkSafeMerge ["system" "environment" "networking" "services" "systemd"] [
                    {
                      ...
                      systemd = {
                        ...
                        services = {
                          ...
                          # redis = config.systemd.services.redis-unbound-cachedb;
                      ...
                      services = let
                        redis-sock = "/run/redis/redis.sock";
                    ...
                    # ((_: {
                    #     systemd.services =
                    #       lib.trace (lib.attrNames finalRedisConfig.config.systemd.services.redis-unbound-cachedb)
                    #       {};
                    #   })
                    #   null)
                  ];
