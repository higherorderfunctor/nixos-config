# This file was generated by nvfetcher, please do not modify it manually.
{ fetchgit, fetchurl, fetchFromGitHub, dockerTools }:
{
  amdvlk = {
    pname = "amdvlk";
    version = "66d23ef671b19b488dddd2408954737be4b53458";
    src = fetchgit {
      url = "https://github.com/GPUOpen-Drivers/AMDVLK.git";
      rev = "66d23ef671b19b488dddd2408954737be4b53458";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-wZ6iRCaK0cA225KMTNvt5o9EXFmZvuXwbADMW2e8fpw=";
    };
    date = "2025-04-30";
  };
  arkenfox = {
    pname = "arkenfox";
    version = "135.0";
    src = fetchFromGitHub {
      owner = "arkenfox";
      repo = "user.js";
      rev = "135.0";
      fetchSubmodules = false;
      sha256 = "sha256-21DoV4SMueMFRHMsvfsPfQIOtsvRWNY06rE4gB7xFnc=";
    };
  };
  bruno-deb-x86_64-linux = {
    pname = "bruno-deb-x86_64-linux";
    version = "2.7.0";
    src = fetchurl {
      url = "https://github.com/usebruno/bruno/releases/download/v2.7.0/bruno_2.7.0_amd64_linux.deb";
      name = "bruno-x86_64-linux.zip";
      sha256 = "sha256-Jka/1t8jWGZbol75KUijykYvUInmHXXQP+pl9C6/+Rc=";
    };
  };
  btop = {
    pname = "btop";
    version = "v1.4.4";
    src = fetchFromGitHub {
      owner = "aristocratos";
      repo = "btop";
      rev = "v1.4.4";
      fetchSubmodules = false;
      sha256 = "sha256-4H9UjewJ7UFQtTQYwvHZL3ecPiChpfT6LEZwbdBCIa0=";
    };
  };
  bun-x86_64-linux = {
    pname = "bun-x86_64-linux";
    version = "1.2.19";
    src = fetchurl {
      url = "https://github.com/oven-sh/bun/releases/download/bun-v1.2.19/bun-linux-x64.zip";
      name = "bun-x86_64-linux.zip";
      sha256 = "sha256-w9PBTppeyD/2fQrP525DFa0G2p809Z/HsTgTeCyvH2Y=";
    };
  };
  catppuccin-btop = {
    pname = "catppuccin-btop";
    version = "1.0.0";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "btop";
      rev = "1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-J3UezOQMDdxpflGax0rGBF/XMiKqdqZXuX4KMVGTxFk=";
    };
  };
  catppuccin-gtk = {
    pname = "catppuccin-gtk";
    version = "23b52b5b9cde1e11c07315e79a55804d2ac77e3a";
    src = fetchgit {
      url = "https://github.com/catppuccin/gtk.git";
      rev = "23b52b5b9cde1e11c07315e79a55804d2ac77e3a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-4tBGN+Rra6C2pjkoLWmNqaYUwpzALsYrn4tkYGUtQl4=";
    };
    date = "2024-06-02";
  };
  catppuccin-hyprland = {
    pname = "catppuccin-hyprland";
    version = "c388ac55563ddeea0afe9df79d4bfff0096b146b";
    src = fetchgit {
      url = "https://github.com/catppuccin/hyprland.git";
      rev = "c388ac55563ddeea0afe9df79d4bfff0096b146b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-xSa/z0Pu+ioZ0gFH9qSo9P94NPkEMovstm1avJ7rvzM=";
    };
    date = "2024-06-19";
  };
  catppuccin-hyprlock = {
    pname = "catppuccin-hyprlock";
    version = "f650895064ae80db7c0e095829fce83fd85d0b26";
    src = fetchgit {
      url = "https://github.com/catppuccin/hyprlock.git";
      rev = "f650895064ae80db7c0e095829fce83fd85d0b26";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-kgVlPaWeaH/p0qGc1+Lj2H6YlDAk5CemNo1FFF8ymZ8=";
    };
    date = "2025-06-02";
  };
  dns-root-hints = {
    pname = "dns-root-hints";
    version = "20250626";
    src = fetchurl {
      url = "https://www.internic.net/domain/named.root";
      sha256 = "sha256-0hIwAdAtBWF5sGxGVvGBghozgFVGHlYITOTS6cX+OfU=";
    };
  };
  fblog = {
    pname = "fblog";
    version = "v4.14.0";
    src = fetchFromGitHub {
      owner = "brocode";
      repo = "fblog";
      rev = "v4.14.0";
      fetchSubmodules = false;
      sha256 = "sha256-ImuNl7ERM7auuGkPHJ93unrqgXRCUBLAuG8OyJXFqms=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./fblog-v4.14.0/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  gluetun-src = {
    pname = "gluetun-src";
    version = "v3.40.0";
    src = fetchFromGitHub {
      owner = "qdm12";
      repo = "gluetun";
      rev = "v3.40.0";
      fetchSubmodules = false;
      sha256 = "sha256-CiA+vCKsjMx+LHVYzHh8x+RFY1rrkkZw5/XU/20it2A=";
    };
  };
  go = {
    pname = "go";
    version = "1.24.5";
    src = fetchurl {
      url = "https://go.dev/dl/go1.24.5.src.tar.gz";
      sha256 = "sha256-dP2wnyNS4rJbeUPlaDbJtHNj0o3sHItWxKlXDzC49Z8=";
    };
  };
  oh-my-posh = {
    pname = "oh-my-posh";
    version = "v26.17.0";
    src = fetchFromGitHub {
      owner = "JanDeDobbeleer";
      repo = "oh-my-posh";
      rev = "v26.17.0";
      fetchSubmodules = false;
      sha256 = "sha256-LQ7lD7u0OM4ksD3saqiMabxvUHs88ce0/Co0+FcCiS0=";
    };
  };
  otel-desktop-viewer = {
    pname = "otel-desktop-viewer";
    version = "12e8a82514b3303be5ebaeb5d02892187bbbe292";
    src = fetchgit {
      url = "https://github.com/CtrlSpice/otel-desktop-viewer.git";
      rev = "12e8a82514b3303be5ebaeb5d02892187bbbe292";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-VqGJWer2FONTcgZdIIs6HNx2bZZJKKUAnhhePq/Vbi4=";
    };
    date = "2025-06-15";
  };
  otel-tui-x86_64-linux = {
    pname = "otel-tui-x86_64-linux";
    version = "0.5.3";
    src = fetchurl {
      url = "https://github.com/ymtdzzz/otel-tui/releases/download/v0.5.3/otel-tui_Linux_x86_64.tar.gz";
      sha256 = "sha256-mIMtHlxlINjioPdD7uyngs/WYaad3zWT3RhEjFov70U=";
    };
  };
  oxc = {
    pname = "oxc";
    version = "1.8.0";
    src = fetchurl {
      url = "https://github.com/oxc-project/oxc/archive/refs/tags/oxlint_v1.8.0.tar.gz";
      sha256 = "sha256-I4ZZsfROa2LnjgReiT07RrJxTZhsGjx0eLCIsF18orE=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./oxc-1.8.0/Cargo.lock;
      outputHashes = {
        
      };
    };
  };
  pnpm = {
    pname = "pnpm";
    version = "10.13.1";
    src = fetchurl {
      url = "https://registry.npmjs.org/pnpm/-/pnpm-10.13.1.tgz";
      name = "pnpm.tgz";
      sha256 = "sha256-D57UjYCJlq4AeDX7XEZBz5owDe8u3cnpV9m75HaMXyg=";
    };
  };
  spectral = {
    pname = "spectral";
    version = "v6.15.0";
    src = fetchFromGitHub {
      owner = "stoplightio";
      repo = "spectral";
      rev = "v6.15.0";
      fetchSubmodules = false;
      sha256 = "sha256-6ywvyZe0ol2B7ZMS/9zWkDKu4u/9dh2fsPegJ6FlLAs=";
    };
  };
  spectral-language-server = {
    pname = "spectral-language-server";
    version = "v1.1.4";
    src = fetchFromGitHub {
      owner = "stoplightio";
      repo = "vscode-spectral";
      rev = "v1.1.4";
      fetchSubmodules = false;
      sha256 = "sha256-0p5aFzaddgSKM62R5Y0K10eCNo9CJkGuJZBhJ2cUF/w=";
    };
  };
  standardnotes-desktop-deb-x86_64-linux = {
    pname = "standardnotes-desktop-deb-x86_64-linux";
    version = "3.198.7";
    src = fetchurl {
      url = "https://github.com/standardnotes/app/releases/download/@standardnotes/desktop@3.198.7/standard-notes-3.198.7-linux-amd64.deb";
      name = "standardnotes-desktop-x86_64-linux.deb";
      sha256 = "sha256-ULiVShgHaO9MD6i/MVfHfyG2rFw1Oqpycjpo/FIrshw=";
    };
  };
  tree-sitter-cql = {
    pname = "tree-sitter-cql";
    version = "v0.2.0";
    src = fetchFromGitHub {
      owner = "shotover";
      repo = "tree-sitter-cql";
      rev = "v0.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-1wJeVajNifxEVe00/YbEyWOsw2ffTjhGfGQnd6a/UhE=";
    };
  };
  vivid-plasma-themes = {
    pname = "vivid-plasma-themes";
    version = "1a0fa78ae85f8204d7d8bd96019c9fe6b044eba9";
    src = fetchgit {
      url = "https://github.com/L4ki/Vivid-Plasma-Themes.git";
      rev = "1a0fa78ae85f8204d7d8bd96019c9fe6b044eba9";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-hqeAFcMDZMm9KwZmnPhnhssnFnNqBvLxnpyhS1epRHs=";
    };
    date = "2025-07-21";
  };
}
