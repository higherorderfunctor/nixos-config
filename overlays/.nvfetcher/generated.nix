# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}: {
  amdvlk = {
    pname = "amdvlk";
    version = "e03007d96a4ff5a508b11e0c6bd7ee09a6728eca";
    src = fetchgit {
      url = "https://github.com/GPUOpen-Drivers/AMDVLK.git";
      rev = "e03007d96a4ff5a508b11e0c6bd7ee09a6728eca";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [];
      sha256 = "sha256-k7zcNkySSoTgq9IDZ4ABGMXwlG4FgVvLGJjd7fkFQ/o=";
    };
    date = "2024-12-05";
  };
  arkenfox = {
    pname = "arkenfox";
    version = "128.0";
    src = fetchFromGitHub {
      owner = "arkenfox";
      repo = "user.js";
      rev = "128.0";
      fetchSubmodules = false;
      sha256 = "sha256-XLswsa2u2qsUPsLYRYn6lwis7VwSjAW6BRNdI4brbSM=";
    };
  };
  bruno-deb-x86_64-linux = {
    pname = "bruno-deb-x86_64-linux";
    version = "1.36.1";
    src = fetchurl {
      url = "https://github.com/usebruno/bruno/releases/download/v1.36.1/bruno_1.36.1_amd64_linux.deb";
      name = "bruno-x86_64-linux.zip";
      sha256 = "sha256-tVT6+8bAUDH2tjkSWDwkcA2pap9Wjf9jW+NVU4VJ4M0=";
    };
  };
  btop = {
    pname = "btop";
    version = "v1.4.0";
    src = fetchFromGitHub {
      owner = "aristocratos";
      repo = "btop";
      rev = "v1.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-A5hOBxj8tKlkHd8zDHfDoU6fIu8gDpt3/usbiDk0/G0=";
    };
  };
  bun-x86_64-linux = {
    pname = "bun-x86_64-linux";
    version = "1.1.40";
    src = fetchurl {
      url = "https://github.com/oven-sh/bun/releases/download/bun-v1.1.40/bun-linux-x64.zip";
      name = "bun-x86_64-linux.zip";
      sha256 = "sha256-n5BFNl4hxkZWiWZ8GLFpUZxQYh2+VbCQjidFyAqhMQI=";
    };
  };
  catppuccin-btop = {
    pname = "catppuccin-btop";
    version = "1.0.0";
    src = fetchFromGitHub {
      owner = "catppuccin";
      repo = "btop";
      rev = "1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-J3UezOQMDdxpflGax0rGBF/XMiKqdqZXuX4KMVGTxFk=";
    };
  };
  catppuccin-gtk = {
    pname = "catppuccin-gtk";
    version = "23b52b5b9cde1e11c07315e79a55804d2ac77e3a";
    src = fetchgit {
      url = "https://github.com/catppuccin/gtk.git";
      rev = "23b52b5b9cde1e11c07315e79a55804d2ac77e3a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [];
      sha256 = "sha256-4tBGN+Rra6C2pjkoLWmNqaYUwpzALsYrn4tkYGUtQl4=";
    };
    date = "2024-06-02";
  };
  catppuccin-hyprland = {
    pname = "catppuccin-hyprland";
    version = "c388ac55563ddeea0afe9df79d4bfff0096b146b";
    src = fetchgit {
      url = "https://github.com/catppuccin/hyprland.git";
      rev = "c388ac55563ddeea0afe9df79d4bfff0096b146b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [];
      sha256 = "sha256-xSa/z0Pu+ioZ0gFH9qSo9P94NPkEMovstm1avJ7rvzM=";
    };
    date = "2024-06-19";
  };
  catppuccin-hyprlock = {
    pname = "catppuccin-hyprlock";
    version = "958e70b1cd8799defd16dee070d07f977d4fd76b";
    src = fetchgit {
      url = "https://github.com/catppuccin/hyprlock.git";
      rev = "958e70b1cd8799defd16dee070d07f977d4fd76b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [];
      sha256 = "sha256-l4CbAUeb/Tg603QnZ/VWxuGqRBztpHN0HLX/h8ndc5w=";
    };
    date = "2024-08-06";
  };
  dns-root-hints = {
    pname = "dns-root-hints";
    version = "20241218";
    src = fetchurl {
      url = "https://www.internic.net/domain/named.root";
      sha256 = "sha256-Q6fzqK8cPPuEF75BvyV16JcKhzvqFNEjEuER3SZ1R6U=";
    };
  };
  fblog = {
    pname = "fblog";
    version = "v4.13.1";
    src = fetchFromGitHub {
      owner = "brocode";
      repo = "fblog";
      rev = "v4.13.1";
      fetchSubmodules = false;
      sha256 = "sha256-YOHLw8YCgOGB1Nn2tD+EnicKd/tiMk07OWv+49btbpw=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./fblog-v4.13.1/Cargo.lock;
      outputHashes = {
      };
    };
  };
  gluetun = {
    pname = "gluetun";
    version = "latest";
    src = dockerTools.pullImage {
      imageName = "qmcgaw/gluetun";
      imageDigest = "sha256:aaaf6c7c108f7f7caae978499b4cb823a135324992ce7cea4acd405a29298141";
      sha256 = "sha256-BTICvfWnHiU77CS4IkF63RyPQN4ZhX7c1wWNCMl6Oxc=";
      finalImageTag = "latest";
      os = "linux";
      arch = "amd64";
    };
  };
  gluetun-src = {
    pname = "gluetun-src";
    version = "v3.39.1";
    src = fetchFromGitHub {
      owner = "qdm12";
      repo = "gluetun";
      rev = "v3.39.1";
      fetchSubmodules = false;
      sha256 = "sha256-L70bpmcb7pehKyCYElLEyliLYLetfsmJ524406MJYCM=";
    };
  };
  go = {
    pname = "go";
    version = "1.23.4";
    src = fetchurl {
      url = "https://go.dev/dl/go1.23.4.src.tar.gz";
      sha256 = "sha256-rTRaxCHpCBQpOpaZzKGd1SOCUcP2h5gLvK4oSVsmNTE=";
    };
  };
  oh-my-posh = {
    pname = "oh-my-posh";
    version = "v24.15.1";
    src = fetchFromGitHub {
      owner = "JanDeDobbeleer";
      repo = "oh-my-posh";
      rev = "v24.15.1";
      fetchSubmodules = false;
      sha256 = "sha256-exrU6MfI55ZE6J4caALJLTVghM+g5MJzS1E6tbS57tM=";
    };
  };
  spectral = {
    pname = "spectral";
    version = "v6.14.2";
    src = fetchFromGitHub {
      owner = "stoplightio";
      repo = "spectral";
      rev = "v6.14.2";
      fetchSubmodules = false;
      sha256 = "sha256-yxT+a1XD6HKllCYSpW4/R26ufDwDDdapccf/5nfFBlw=";
    };
  };
  standardnotes-desktop-deb-x86_64-linux = {
    pname = "standardnotes-desktop-deb-x86_64-linux";
    version = "3.195.14";
    src = fetchurl {
      url = "https://github.com/standardnotes/app/releases/download/@standardnotes/desktop@3.195.14/standard-notes-3.195.14-linux-amd64.deb";
      name = "standardnotes-desktop-x86_64-linux.deb";
      sha256 = "sha256-BdzlkvWJo1uCjPZEhftf4WV1FvlujhOA9BO6RUWENic=";
    };
  };
  tree-sitter-cql = {
    pname = "tree-sitter-cql";
    version = "v0.2.0";
    src = fetchFromGitHub {
      owner = "shotover";
      repo = "tree-sitter-cql";
      rev = "v0.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-1wJeVajNifxEVe00/YbEyWOsw2ffTjhGfGQnd6a/UhE=";
    };
  };
  vivid-plasma-themes = {
    pname = "vivid-plasma-themes";
    version = "c79265c041de5cd5e8e32817c012e5a76b48717f";
    src = fetchgit {
      url = "https://github.com/L4ki/Vivid-Plasma-Themes.git";
      rev = "c79265c041de5cd5e8e32817c012e5a76b48717f";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [];
      sha256 = "sha256-xzf9YIwI0oUdWTnAK+hV4Em7Z10HKT1HWutYioXhO1Y=";
    };
    date = "2024-12-14";
  };
  wezterm = {
    pname = "wezterm";
    version = "6f375e29a2c4d70b8b51956edd494693196c6692";
    src = fetchgit {
      url = "https://github.com/wez/wezterm.git";
      rev = "6f375e29a2c4d70b8b51956edd494693196c6692";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [];
      sha256 = "sha256-YbomxkYXeX4XoCAKZn5mIV6jLWLRetFMOmYFclSQmyM=";
    };
    cargoLock."Cargo.lock" = {
      lockFile = ./wezterm-6f375e29a2c4d70b8b51956edd494693196c6692/Cargo.lock;
      outputHashes = {
        "xcb-imdkit-0.3.0" = "sha256-77KaJO+QJWy3tJ9AF1TXKaQHpoVOfGIRqteyqpQaSWo=";
        "sqlite-cache-0.1.3" = "sha256-sBAC8MsQZgH+dcWpoxzq9iw5078vwzCijgyQnMOWIkk=";
      };
    };
    date = "2024-12-05";
  };
}
