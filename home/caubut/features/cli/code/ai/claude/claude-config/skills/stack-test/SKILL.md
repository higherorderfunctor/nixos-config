...
2. **Parse arguments**. If no command is provided, detect from the project:
...
3. **Determine mode**. If `--fix` is in arguments, use fix mode. Otherwise
   use run mode.

4. **Determine target revset**. If a revset argument is provided, use it.
   Otherwise default to `stack()`.

### Run Mode (testing)

5. **Run tests across the stack**:
   ```bash
...
5. **Apply formatter across the stack**:
   ```bash
...
6. **Verify** with `git sl` to show any commits that were modified.

7. **Optionally re-run tests** to confirm the formatting didn't break anything:
   ```bash
