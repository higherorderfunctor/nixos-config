...
/*
    ...
    if (error != NULL) {
        g_printerr("Error querying file info: %s\n", error->message);
        g_error_free(error);
        return 1;
    }

    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_ACCESS_CAN_READ)) {
        g_print("File can be read\n");
    } else {
        g_print("File cannot be read\n");
    }

    if (g_file_info_has_attribute(info, G_FILE_ATTRIBUTE_ACCESS_CAN_WRITE)) {
        g_print("File can be written\n");
    } else {
        g_print("File cannot be written\n");
    }
*/

// enum Direction {
//   F_OK = 0,
//   R_OK = 2, FILE_ATTRIBUTE_ACCESS_CAN_READ
//   W_OK = 4, FILE_ATTRIBUTE_ACCESS_CAN_WRITE
//   Right,
// }
...
//     type T = Gio.AsyncReadyCallback
//     const z = file.query_info_async.bind(file);
//     const queryInfo = effectify(
//       file.query_info_async.bind(file), (error, args) => {})
//       // handleErrnoException('FileSystem', 'access'),
//       // handleBadArgument('access'),
//     );
//
//     file.query_info_async(attributes, Gio.FileQueryInfoFlags.NONE, GLib.PRIORITY_DEFAULT, null);
