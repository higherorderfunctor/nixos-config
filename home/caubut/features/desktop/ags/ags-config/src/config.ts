  ...
  Widget.Label({
    ...
    setup: (self) =>
      self
        ...
        .poll(1000, (self) => execAsync(['date', '+%H:%M:%S %b %e.']).then((date) => (self.label = date))),
  });
...
const BatteryLabel = () =>
  Widget.Box({
    ...
    children: [
      ...
      Widget.ProgressBar({
        vpack: 'center',
        fraction: Battery.bind('percent').transform((p) => (p > 0 ? p / 100 : 0)),
      }),
    ],
  });
...
monitorFile(`${App.configDir}/style.css`, () => {
  App.resetCss();
